...
function getArchiverLocalPath(id: string) {
...
function listFixedHeyMateVideos() {
  ...
  for (const video of fixedHeyMateDb.data.videos) {
    ...
    if (video.status === 'archiving') continue;
  ...
  if (modified) fixedHeyMateDb.save();
...
async function startFixedHeyMateArchive(url: string) {
  ...
  if (existing) {
    if (existing.storage_type !== 'local') return existing;
...
function normalizeTikTokUsername(raw: string): string | null {
  const stripped = raw.replace(/^@/, '').trim();
  // Allow only safe TikTok username characters: alphanumeric, dot, underscore
  if (!/^[A-Za-z0-9._]{1,64}$/.test(stripped)) return null;
  // Reject edge-case traversal values just in case
  if (stripped === '.' || stripped === '..') return null;
  return stripped;
}

function isWithinTikTokDownloaderDir(filePath: string): boolean {
...
function startTikTokDownloadJob(username: string) {
  ...
  if (!cleanUsername) throw new Error('Invalid username format');
...
async function startServer() {
  ...
  const playArchiverVideo = (req: express.Request, res: express.Response) => {
    const video = getFixedHeyMateVideo(req.params.id);
    if (!video) return res.status(404).json({ error: 'Video not found' });
  ...
  app.get('/api/tiktok-downloader/jobs/:id/download', (req, res) => {
    ...
    if (!isWithinTikTokDownloaderDir(job.zip_path)) return res.status(403).json({ error: 'Forbidden' });
  ...
  app.delete('/api/tiktok-downloader/jobs/:id', (req, res) => {
    ...
    if (job.output_dir && fs.existsSync(job.output_dir)) {
      if (!isWithinTikTokDownloaderDir(job.output_dir)) return res.status(403).json({ error: 'Forbidden' });
