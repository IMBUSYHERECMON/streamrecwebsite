...
const fixedHeyMateDb = new JsonDB('fixedheymate-db.json', {
    videos: []
});
const tiktokDownloaderDb = new JsonDB('tiktok-downloader-db.json', {
    jobs: []
});
...
function extractGoFileCode(url) {
    if (!url)
        return null;
...
async function fetchGoFileDirectLink(downloadPage, guestToken) {
    ...
    for (const attempt of attempts) {
        try {
            ...
            const firstVideo = Object.values(children).find((child) => child?.mimetype?.startsWith('video/') || String(child?.name || '').toLowerCase().endsWith('.mp4'));
            if (firstVideo?.link)
...
const activeDownloads = new Map(); // Map<streamerId, child_process>
const activeConversions = new Map();
const MANUAL_STOP_COOLDOWN_MS = 90_000;
const bundledFfmpegPath = typeof ffmpegStatic === 'string' ? ffmpegStatic : null;
const streamlinkPath = path.join(APP_ROOT, '.venv', 'bin', 'streamlink');
...
function normalizeStreamerInput(raw) {
    ...
    try {
        ...
        if (host.includes('youtube.com') || host.includes('youtu.be')) {
            if (cleanPath.includes('/live'))
                return parsed.toString();
            if (firstSegment.startsWith('@'))
                return `https://www.youtube.com/${firstSegment}/live`;
            return parsed.toString();
        }
        return parsed.toString();
    }
...
function shouldUseStreamlink(raw) {
    // Streamlink is the reliable path in this environment for Twitch/YouTube/TikTok live URLs.
    return isTwitchUrl(raw) || isYouTubeUrl(raw) || isTikTokUrl(raw);
}
function getRequiredRecorderForUrl(raw) {
...
function getRecordingById(id) {
    return db.data.recordings.find((r) => r.id === id);
}
...
function getArchiverLocalPath(id) {
    return path.join(FIXED_HEYMATE_STORAGE_DIR, 'videos', `${id}.mp4`);
}
...
function seedFixedHeyMateDb() {
    if (fixedHeyMateDb.data.videos.length > 0)
        return;
    const now = new Date();
...
function listFixedHeyMateVideos() {
    ...
    for (const video of fixedHeyMateDb.data.videos) {
        ...
        if (video.status === 'archiving')
            continue;
        const localPath = getArchiverLocalPath(video.id);
        if (fs.existsSync(localPath)) {
            const size = fs.statSync(localPath).size;
            if (video.status !== 'ready' || video.error_message || video.file_size !== size) {
                video.status = 'ready';
    ...
    if (modified)
        fixedHeyMateDb.save();
    const toTimestamp = (uploadDate, archivedAt) => {
        ...
        if (!Number.isNaN(parsedArchived))
...
function getFixedHeyMateVideo(id) {
    return fixedHeyMateDb.data.videos.find((video) => video.id === id);
}
function updateFixedHeyMateVideo(id, patch) {
...
async function fetchFixedHeyMateMeta(url) {
...
async function startFixedHeyMateArchive(url) {
    ...
    if (existing) {
        if (existing.storage_type !== 'local')
            return existing;
        const localPath = getArchiverLocalPath(existing.id);
        if (fs.existsSync(localPath)) {
    ...
    args.push('--js-runtimes', `node:${process.execPath}`, '-f', 'best[ext=mp4]/best', '-o', filePath, url);
...
function getTikTokJob(id) {
    return tiktokDownloaderDb.data.jobs.find((job) => job.id === id);
}
function updateTikTokJob(id, patch) {
...
function normalizeTikTokUsername(raw) {
    ...
    // Allow only safe TikTok username characters: alphanumeric, dot, underscore
    if (!/^[A-Za-z0-9._]{1,64}$/.test(stripped))
        return null;
    // Reject edge-case traversal values just in case
    if (stripped === '.' || stripped === '..')
        return null;
    return stripped;
}
function isWithinTikTokDownloaderDir(filePath) {
    const resolved = path.resolve(filePath);
    const base = path.resolve(TIKTOK_DOWNLOADER_DATA_DIR);
    return resolved.startsWith(base + path.sep) || resolved === base;
}
function startTikTokDownloadJob(username) {
...
async function ensureRecordingDirectUrl(recording) {
    if (isDirectMediaUrl(recording.direct_url))
        return recording.direct_url;
...
function getPlaybackFilePath(filePath) {
...
function deleteRecordingArtifacts(recording) {
...
async function ensurePlaybackFile(recording) {
    ...
    const conversion = new Promise((resolve, reject) => {
        ...
        const fallbackTranscode = () => {
            execFile(bundledFfmpegPath, ['-y', '-i', recording.file_path, '-movflags', '+faststart', '-c:v', 'libx264', '-c:a', 'aac', outputPath], (fallbackError) => {
                activeConversions.delete(recording.id);
                if (fallbackError)
        ...
        execFile(bundledFfmpegPath, ['-y', '-i', recording.file_path, '-movflags', '+faststart', '-c', 'copy', outputPath], (copyError) => {
            if (!copyError)
...
function setStreamerManualStop(streamerId) {
    const streamer = db.data.streamers.find((s) => s.id === streamerId);
    if (!streamer)
        return;
...
function finalizeRecordingProcess(streamer, recordingId, filePath, safeName, code, signal, isLive, notLiveDetected) {
    ...
    if (isLive) {
        ...
        if (recording && recording.status !== 'Completed') {
            ...
            if (notLiveDetected || (code !== 0 && signal !== 'SIGTERM' && signal !== 'SIGKILL' && finalSize < MIN_VALID_RECORDING_BYTES)) {
                db.data.recordings = db.data.recordings.filter((r) => r.id !== recordingId);
                if (fs.existsSync(filePath)) {
                    try {
                        fs.unlinkSync(filePath);
                    }
...
function startActualRecording(streamer) {
    if (activeDownloads.has(streamer.id))
        return; // Already recording/checking
    const dependencies = getRecorderDependencies();
    ...
    if (shouldUseStreamlink(normalizedUrl)) {
        ...
        dlProcess.stderr.on('data', (data) => {
            ...
            if (!isLive && (lowered.includes('no playable streams found') ||
            ...
            if (msg)
                console.log(`[${safeName}] streamlink: ${msg}`);
        });
...
function cleanupStaleRecordings() {
    let modified = false;
    db.data.streamers.forEach((s) => {
        if (s.state !== 'offline') {
            s.state = 'offline';
            modified = true;
        }
    });
    db.data.recordings.forEach((r) => {
...
async function startServer() {
    ...
    app.post('/api/streamers/:id/start', (req, res) => {
        ...
        const recorder = getRecorderReadiness();
        const normalizedUrl = normalizeStreamerInput(streamer.url);
        const required = getRequiredRecorderForUrl(normalizedUrl);
        const missing = required.filter((key) => !recorder.dependencies[key].ready);
        if (missing.length > 0) {
            return res.status(503).json({
                error: `Recorder dependencies missing: ${missing.join(', ')}`,
                recorder
            });
        }
        streamer.url = normalizedUrl;
        streamer.manual_start_required = false;
        streamer._last_check_at = Date.now();
        startActualRecording(streamer);
        db.save();
    ...
    app.post('/api/streamers/:id/stop', (req, res) => {
        ...
        const activeRecording = db.data.recordings.find((r) => r.streamer_id === streamer.id && r.status === 'Recording');
        if (activeRecording) {
            ...
            if (fs.existsSync(partFile)) {
                try {
                    fs.renameSync(partFile, activeRecording.file_path);
                }
                catch (e) { }
            }
        }
    ...
    app.get('/api/recordings', (req, res) => {
        ...
        const records = db.data.recordings.map((r) => {
    ...
    app.delete('/api/recordings/:id', (req, res) => {
        ...
        if (recording.status === 'Recording')
            return res.status(409).json({ error: 'Cannot delete active recording' });
        deleteRecordingArtifacts(recording);
        db.data.recordings = db.data.recordings.filter((r) => r.id !== req.params.id);
        activeConversions.delete(recording.id);
        db.save();
        res.json({ success: true });
    });
    app.delete('/api/recordings', (req, res) => {
        const deletable = db.data.recordings.filter((r) => r.status !== 'Recording');
        for (const recording of deletable) {
            deleteRecordingArtifacts(recording);
            activeConversions.delete(recording.id);
        }
        db.data.recordings = db.data.recordings.filter((r) => r.status === 'Recording');
        db.save();
    ...
    app.post('/api/recordings/:id/resolve-direct-url', async (req, res) => {
        ...
        if (!recording)
            return res.status(404).json({ error: 'Recording not found' });
        if (!recording.file_url)
            return res.status(400).json({ error: 'No GoFile url to resolve' });
    ...
    const sendArchiverImportSummary = (req, res) => {
        const summary = readFixedHeyMateSummary();
        if (!summary)
            return res.status(404).json({ error: 'Archiver import not found' });
        res.json(summary);
    };
    ...
    app.get('/api/imports/tiktok-downloader', (req, res) => {
        ...
        if (!summary)
            return res.status(404).json({ error: 'TikTok downloader script not found' });
        res.json(summary);
    });
    const listArchiverVideos = (req, res) => {
        res.json(listFixedHeyMateVideos());
    };
    app.get('/api/archiver/videos', listArchiverVideos);
    app.get('/api/fixedheymate/videos', listArchiverVideos);
    const getArchiverVideo = (req, res) => {
    ...
    const archiveIntoArchiver = async (req, res) => {
        const { url } = req.body;
    ...
    const playArchiverVideo = (req, res) => {
        const video = getFixedHeyMateVideo(req.params.id);
        if (!video)
            return res.status(404).json({ error: 'Video not found' });
        if (video.storage_type === 'remote') {
            if (!video.playback_url || !/^https?:\/\//i.test(video.playback_url)) {
                updateFixedHeyMateVideo(video.id, {
                    status: 'error',
                    error_message: 'Remote playback URL is missing or invalid'
                });
                return res.status(404).json({ error: 'Remote playback URL is missing or invalid' });
            }
            return axios.head(video.playback_url, { maxRedirects: 5, validateStatus: () => true })
                .then((upstream) => {
                if (upstream.status >= 400) {
                    updateFixedHeyMateVideo(video.id, {
                        status: 'error',
                        error_message: `Remote link unavailable (HTTP ${upstream.status})`
                    });
                    return res.status(502).json({ error: `Remote link unavailable (HTTP ${upstream.status})` });
                }
                res.redirect(302, video.playback_url);
            })
                .catch(() => {
                updateFixedHeyMateVideo(video.id, {
                    status: 'error',
                    error_message: 'Remote link unavailable'
                });
                res.status(502).json({ error: 'Remote link unavailable' });
            });
        }
        const localPath = getArchiverLocalPath(video.id);
        if (!fs.existsSync(localPath)) {
            if (video.source_url) {
                updateFixedHeyMateVideo(video.id, {
                    status: 'archiving',
                    error_message: 'Local file missing. Re-archiving from source...'
                });
                void startFixedHeyMateArchive(video.source_url).catch((error) => {
                    updateFixedHeyMateVideo(video.id, {
                        status: 'error',
                        error_message: error instanceof Error ? error.message : 'Re-archive failed'
                    });
                });
                return res.status(409).json({ error: 'Local archive missing. Re-archiving started.' });
            }
            updateFixedHeyMateVideo(video.id, {
                status: 'error',
                error_message: 'Local archive file missing'
            });
    ...
    app.post('/api/tiktok-downloader/jobs', (req, res) => {
        ...
        if (!normalizeTikTokUsername(username)) {
        ...
        try {
            ...
            res.status(400).json({ error: e.message || 'Failed to start job' });
        }
    });
    ...
    app.get('/api/tiktok-downloader/jobs/:id/download', (req, res) => {
        ...
        if (!isWithinTikTokDownloaderDir(job.zip_path))
    ...
    app.delete('/api/tiktok-downloader/jobs/:id', (req, res) => {
        ...
        if (job.output_dir && fs.existsSync(job.output_dir)) {
            if (!isWithinTikTokDownloaderDir(job.output_dir))
                return res.status(403).json({ error: 'Forbidden' });
            try {
    ...
    app.post('/api/settings', (req, res) => {
        ...
        if (webhook_url !== undefined)
            db.data.settings.webhook_url = webhook_url;
