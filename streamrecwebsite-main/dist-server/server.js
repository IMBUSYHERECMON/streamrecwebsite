...
async function downloadDirectVideoToLocal(url, outputPath) {
...
function toArchiverResponseVideo(video) {
    return {
        ...video,
...
async function redownloadArchiverVideo(id) {
    const video = getFixedHeyMateVideo(id);
    if (!video)
...
async function createApp() {
    ...
    const redownloadArchiver = async (req, res) => {
        try {
    ...
    const playArchiverVideo = async (req, res) => {
        reconcileArchiverVideoStatuses();
        const video = getFixedHeyMateVideo(req.params.id);
        if (!video)
            return res.status(404).json({ error: 'Video not found' });
