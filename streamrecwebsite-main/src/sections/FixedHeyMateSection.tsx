...
export function FixedHeyMateSection() {
  ...
  const redownloadVideo = async () => {
    if (!selectedVideo) return;
    setIsRedownloading(true);
    try {
      await api.post<FixedHeyMateVideo>(`/api/archiver/videos/${selectedVideo.id}/redownload`);
      await loadVideos();
      setError(null);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to re-download video.');
    } finally {
                ...
                <>
                  ...
                  <article className="card stack-md">
                    <div className="space-between">
                      ...
                      <div className="vault-toggle">
                        <button className="btn" onClick={() => void redownloadVideo()} disabled={isRedownloading}>
                          {isRedownloading ? <Loader2 size={14} className="spin" /> : <Archive size={14} />}
                          Re-download
                        </button>
